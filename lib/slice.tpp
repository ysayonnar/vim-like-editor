...
template <typename T>
Slice<T>::Slice(const Slice &other) : data(new T[other.capacity]), length(other.length), capacity(other.capacity) {
...
template <typename T>
Slice<T>::Slice(Slice &&other) noexcept : data(other.data), length(other.length), capacity(other.capacity) {
    other.data = nullptr;
    other.length = 0;
    other.capacity = 0;
}
...
template <typename T>
typename Slice<T>::Iterator Slice<T>::begin() { return Iterator(data); }

template <typename T>
typename Slice<T>::Iterator Slice<T>::end() { return Iterator(data + length); }
